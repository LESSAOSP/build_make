...
class PayloadSigner(object):
    ...
    def Sign(self, in_file):
        ...
        cmd = [self.signer] + self.signer_args + \
...
def HasRecoveryPatch(target_files_zip, info_dict):
    ...
    board_builds_vendorimage = info_dict.get(
    ...
    for root, subdirs, files in os.walk(install_path):
        for f in files:
            ...
            install_target = os.path.join(
                "install", os.path.relpath(root, install_path), f)
...
""" % bcb_dev)
    ...
    build_date = target_info.GetBuildProp("com.lessaosp.build_date")
    security_patch = target_info.GetBuildProp(
        "ro.build.version.security_patch")
    device = target_info.GetBuildProp("com.lessaosp.device")
    ...
    if CopyInstallTools(output_zip):
        ...
        script.SetPermissionsRecursive(
            "/tmp/install", 0, 0, 0o755, 0o644, None, None)
        script.SetPermissionsRecursive(
            "/tmp/install/bin", 0, 0, 0o755, 0o755, None, None)
...
""" % bcb_dev)
    ...
    FinalizeMetadata(metadata, staging_file,
                     output_file, needed_property_files)
...
class PropertyFiles(object):
    ...
    def Verify(self, input_zip, expected):
        ...
        assert actual == expected, \
            "Mismatching streaming metadata: {} vs {}.".format(
                actual, expected)
...
def FinalizeMetadata(metadata, input_file, output_file, needed_property_files):
    ...
    def ComputeAllPropertyFiles(input_file, needed_property_files):
        ...
        with zipfile.ZipFile(input_file) as input_zip:
            for property_files in needed_property_files:
                metadata[property_files.name] = property_files.Compute(
                    input_zip)
    ...
    with zipfile.ZipFile(output_file) as output_zip:
        for property_files in needed_property_files:
            property_files.Verify(
                output_zip, metadata[property_files.name].strip())
    ...
    if not OPTIONS.two_step:
        if updating_boot:
            if include_full_boot:
                ...
                source_expr = 'concat("{}:",{},":{}:{}")'.format(
                    boot_type, boot_device_expr, source_boot.size, source_boot.sha1)
                script.PatchPartitionExpr(
                    target_expr, source_expr, '"boot.img.p"')
        else:
...
""" % bcb_dev)
    ...
    if OPTIONS.downgrade:
        script.AddToZip(source_zip, output_zip,
...
def GetTargetFilesZipForRetrofitDynamicPartitions(input_file,
    ...
    for src, dst in replace.items():
        assert src in namelist, \
            'Missing {} in {}; {} cannot be written'.format(
                src, input_file, dst)
...
def GenerateAbOtaPackage(target_file, output_file, source_file=None):
    ...
    if source_file is not None:
        target_info = common.BuildInfo(
            OPTIONS.target_info_dict, OPTIONS.oem_dicts)
        source_info = common.BuildInfo(
            OPTIONS.source_info_dict, OPTIONS.oem_dicts)
    else:
        target_info = common.BuildInfo(OPTIONS.info_dict, OPTIONS.oem_dicts)
        source_info = None
    ...
    if OPTIONS.retrofit_dynamic_partitions:
        target_file = GetTargetFilesZipForRetrofitDynamicPartitions(
            target_file, target_info.get(
                "super_block_devices").strip().split(),
