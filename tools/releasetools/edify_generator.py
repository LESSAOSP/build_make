...
class EdifyGenerator(object):
    ...
    def PatchPartitionCheck(self, target, source):
        ...
        """
        self._CheckSecondTokenNotSlotSuffixed(
            target, "PatchPartitionExprCheck")
        self._CheckSecondTokenNotSlotSuffixed(
            source, "PatchPartitionExprCheck")
    ...
    def Mount(self, mount_point, mount_options_by_format=""):
        ...
        if fstab:
            ...
            if p.context is not None:
                mount_flags = p.context + \
                    ("," + mount_flags if mount_flags else "")
    ...
    def WipeBlockDevice(self, partition):
        if partition not in ("/system", "/vendor"):
            raise ValueError(
                ("WipeBlockDevice doesn't work on %s\n") % (partition,))
        fstab = self.fstab
    ...
    def PatchPartition(self, target, source, patch):
        ...
        self.PatchPartitionExpr('"%s"' % target, '"%s"' %
                                source, '"%s"' % patch)
